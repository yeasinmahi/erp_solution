...
namespace Purchase_BLL.Asset
{
    public class AutoSearch_BLL
    {
        ...
        private static SearchTDS.sprAutosearchRequesitionWithoutFGAndSFGDataTable[] tableItemFG = null;
        private static SearchTDS.AutoSearchProductDataTable[] tableProduct = null;
       ...
       public string WhId = String.Empty;
        ...
        public string[] AutoSearchItemFGAndSFD(string whid, string prefix)
        {
            if (tableItem == null || tableItem.Length < 1 || !WhId.Equals(whid))
            {
                tableItemFG = new SearchTDS.sprAutosearchRequesitionWithoutFGAndSFGDataTable[Convert.ToInt32(whid)];
                sprAutosearchRequesitionWithoutFGAndSFGTableAdapter adpCoa = new sprAutosearchRequesitionWithoutFGAndSFGTableAdapter();
                tableItemFG[e] = adpCoa.GetAutoSearch(Convert.ToInt32(whid));
                WhId = whid;
            }
            //Inatialize(int.Parse(whid));
            ...
            if (prefix.Trim().Length >= 3)
            {
                if (prefix == "" || prefix == "*")
                {
                    var rows = from tmp in tableItemFG[e]//Convert.ToInt32(ht[whid])
                               orderby tmp.strItem
                               select tmp;
                    if (rows.Any())
                    {
        ...
        public string[] AutoSearchProduct(int Type, int WHId, string prefix)
        {
            //DataTable tbl = new DataTable();
            try
            {
                AutoSearchProductTableAdapter adpCoa = new AutoSearchProductTableAdapter();
                if (tableProduct == null || tableProduct.Length < 1 || !WhId.Equals(WHId))
                {
                    tableProduct = new SearchTDS.AutoSearchProductDataTable[Convert.ToInt32(WHId)];
                    tableProduct[e] = adpCoa.GetAutoSearchProduct(Type, WHId);
                    WhId = WHId.ToString();
                }
                ...
                if (prefix.Trim().Length >= 3)
                {
                    if (prefix == "" || prefix == "*")
                    {
                        var rows = from tmp in tableProduct[e]//Convert.ToInt32(ht[whid])
                                   orderby tmp.strItem
                                   select tmp;
                        if (rows.Any())
                        {
                ...
                //tbl = adpCoa.GetAutoSearchProduct(Type, WHId);
                if (tbl.Rows.Count > 0)
                {
                    string[] retStr = new string[tbl.Rows.Count];
                    for (int i = 0; i < tbl.Rows.Count; i++)
