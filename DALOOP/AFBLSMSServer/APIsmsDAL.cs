    ...
    public class ApiSmsDal
    {
      ...
      //  DBUtility _db = new DBUtility();
        ...
        public bool InsertApIsms(ApiSmsModel model)
        {
            try
            {
                sprAPITableAdapter adp = new sprAPITableAdapter();
                adp.Insert1(model.UserName, model.Password, model.MaskingClient, model.PhoneNo, model.Message,
                    model.UnitId, model.InsertDate);
                return true;
            }
            catch (Exception e)
            {
