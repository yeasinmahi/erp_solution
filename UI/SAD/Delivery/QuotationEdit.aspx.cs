...
{
    public partial class QuotationEdit : System.Web.UI.Page
    {
            ...
            if (grdvQuationDetails.Rows.Count > 0)
            {
                ...
                for (int index = 0; index < grdvQuationDetails.Rows.Count; index++)
                {
                    ...
                    if (decimal.Parse(quantity) > 0)
                    {
                        ...
                        //unitid = int.Parse("91"); hdnUnitid
                        unitid = int.Parse(Session["intUnitId"].ToString());
                        ...
                        dt = bll.getCOAID(itemid, unitid, intsalestype);
                        int ExtId = 0;
                        dt = bll.getUnitVsLocalSalesType(unitid);

                        //int salestype = 90;
                        int salestype=int.Parse (dt.Rows[0]["inttypeid"].ToString());

                        string ThanaRate = "0";
                        dt = bll.getUOM(int.Parse(Session["intCustid"].ToString()), int.Parse("0"), int.Parse("0"), salestype, Convert.ToDateTime(DateTime.Now));
