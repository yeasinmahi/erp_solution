...
<webopt:BundleReference ID="BundleReference1" runat="server" Path="~/Content/Bundle/hrCSS" />
    <script>
        function Registration(url) {newwindow = window.open(url, 'sub', 'scrollbars=yes,toolbar=0,height=400,width=580,top=50,left=150, close=no');
...
<%--=========================================Start My Code From Here===============================================--%>
    <div class="leaveApplication_container"><b>Corporate Sales Return Amount Review: </b><asp:HiddenField ID="hdnconfirm" runat="server" /><asp:HiddenField ID="hdnenroll" runat="server" /><hr />
   ...
   <tr><td style="text-align:center"><asp:Button ID="btnshow" runat="server" Text="Show" OnClick="btnshow_Click" /></td></tr>
    ...
    <asp:TemplateField HeaderText="Edit"><ItemTemplate><asp:Button ID="btnedit" runat="server" Text="Edit"  CommandArgument='<%#Eval("intCustId") + "," +Eval("strChallanNo")%>' OnClick="btnedit_Click" />
    </ItemTemplate></asp:TemplateField> 
    <asp:TemplateField HeaderText="Review"><ItemTemplate><asp:Button ID="App" runat="server" Text="Review" OnClientClick="Confirm();"  CommandArgument='<%#Eval("intCustId") + "," +Eval("strChallanNo")+ "," +Eval("pk")+ "," +Eval("Total")%>' OnClick="App_Click" />
    </ItemTemplate></asp:TemplateField>  
