...
{
    public partial class ReturnSalesDOCreation : System.Web.UI.Page
    {
        decimal promPrice = 0;
        ...
        protected string GetFullTotal(string pr, string qnt, string ext, string vat)
        {
            decimal discount = decimal.Parse(vat);
            decimal tot = 0, calctotal = 0;
            tot = (decimal.Parse(pr) + decimal.Parse(ext)) * decimal.Parse(qnt);
            calctotal = tot - discount;
            hdndiscount.Value = Convert.ToString(calctotal);
            return CommonClass.GetFormettingNumber(calctotal);
        ...
        protected void btnAdd_Click(object sender, EventArgs e)
        {
            ...
            double discnttaka = Convert.ToDouble(discntamnt);

            double calculteddiscount = 0.00;
            ...
            else { calculteddiscount = Convert.ToDouble("0.00"); }
            ...
            if ((double.Parse(lblBl.Text) - undelvvalue + undelvcommission) <= deccompareprice)
            {
        ...
        protected void btnSubmit_Click(object sender, EventArgs e)
        {
            ...
            SAD_BLL.Sales.SalesOrder se = new SAD_BLL.Sales.SalesOrder();
            se.AddUpdateReturnSalesDO(xml, Session[SessionParams.USER_ID].ToString(), ddlUnit.SelectedValue
                 ...
                 , ddlShip.SelectedValue, RSDO.Checked, ref code, ref id);

            lblError.Text = "Return Sales DO No: " + code;
        ...
        private string GetXmlFilePath()
        {
          ...
          return  Server.MapPath("~/SAD/Sales/Data/SRtnDO_" + Session[SessionParams.USER_ID] + "_" + unit + "_item.xml");
        }
