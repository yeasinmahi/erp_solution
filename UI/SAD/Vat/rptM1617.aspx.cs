...
{
    public partial class rptM1617 : BasePage
    {
        ...
        {
            if (int.Parse(ddlRpttype.SelectedValue) == 1)
            {
                dt = objMush.getM16report(int.Parse(hdnAccno.Value), txtdtefdate.Text, txttodate.Text,1);
                dgvm16.DataSource = dt;
                dgvm16.DataBind();
                dgvm16.Visible = true;
                dgv17.Visible = false;
            }
            ...
            {
                dtefdate = DateTime.Parse(txtdtefdate.Text.ToString()).ToString("yyyy-mm-dd")+""+txtStime.Text;
                dtetdate = DateTime.Parse(txtdtefdate.Text.ToString()).ToString("yyyy-mm-dd") + "" + txtEdate.Text;
              
                dt = objMush.getM16report(int.Parse(hdnAccno.Value), dtefdate, dtetdate,2);
                dgv17.DataSource = dt;
                dgv17.DataBind();
                dgvm16.Visible = false;
                dgv17.Visible = true;
        ...
        protected void dgvdgv17_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            if (e.Row.RowType == DataControlRowType.DataRow)
            {
