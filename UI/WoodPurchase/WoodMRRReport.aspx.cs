...
{
    public partial class WoodMRRReport : BasePage
    {
        ...
        protected void btnShow_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                dt = new DataTable();
                dt = bll.GetWoodReport(dteFromDate, dteToDate, intJobStation, 4, 0);
        ...
        protected void dgvReport_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            try
            {
                ...
                {
                    totalweight += decimal.Parse(((Label)e.Row.Cells[4].FindControl("lblTWeight")).Text);
                    deduction += decimal.Parse(((Label)e.Row.Cells[5].FindControl("lblDeduction")).Text);
                    netweight += decimal.Parse(((Label)e.Row.Cells[6].FindControl("lblNetWeight")).Text);
                    totalamount += decimal.Parse(((Label)e.Row.Cells[7].FindControl("lblTAmount")).Text);
                    unloadingbill += decimal.Parse(((Label)e.Row.Cells[8].FindControl("lblUnloading")).Text);
                    netpayable += decimal.Parse(((Label)e.Row.Cells[9].FindControl("lblNetPayable")).Text);
                }
            }
            catch { }
        }
