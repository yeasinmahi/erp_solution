...
{
    public partial class InternalTReportForAudit : BasePage
    {
        ...
        private void LoadGrid()
        {
            try
            {
                ...
                else if (intReportType == 3)
                {
                    ...
                    Label2.Text = "Trip Wise Details Report For Bill";
                    ...
                    dgvDetailsRExcelToExport.DataSource = "";
                    dgvDetailsRExcelToExport.DataBind();
                    ...
                    dt = new DataTable();
                    dt = obj.GetDateWiseCompleteReport(intWork, intShipPoint, dteFromDate, dteToDate, intFuelStationid, intBillStatus, intEnroll, intReportCategory);
                    dgvDetailsRExcelToExport.DataSource = dt;
                    dgvDetailsRExcelToExport.DataBind();

                    dgvDetailsRExcelToExport.Columns[3].Visible = true;
                    dgvDetailsRExcelToExport.Columns[4].Visible = true;
                    dgvDetailsRExcelToExport.Columns[5].Visible = true;
                    dgvDetailsRExcelToExport.Columns[6].Visible = true;
                    dgvDetailsRExcelToExport.Columns[7].Visible = true;
                    dgvDetailsRExcelToExport.Columns[9].Visible = true;
                    dgvDetailsRExcelToExport.Columns[16].Visible = true;
                    dgvDetailsRExcelToExport.Columns[20].Visible = true;
                    dgvDetailsRExcelToExport.Columns[30].Visible = true;
                ...
                else if (intReportType == 4)
                {
                    ...
                    if (rdoComplete.Checked == true) { intBillStatus = 1; } else { intBillStatus = 0; }
                    intEnroll = 0;
                    intFuelStationid = int.Parse(ddlVehicleSupplier.SelectedValue.ToString());
                    intWork = 24;                    
                    dt = new DataTable();
                    dt = obj.GetDateWiseCompleteReport(intWork, intShipPoint, dteFromDate, dteToDate, intFuelStationid, intBillStatus, intEnroll, intReportCategory);
        ...
        protected void dgvDetailsRExcelToExport_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            try
            {
                if (e.Row.RowType == DataControlRowType.DataRow)
                {
                    ...
                    totaldieseltk += decimal.Parse(((Label)e.Row.Cells[12].FindControl("lblDieselTk")).Text);
                    totaldieseltk += decimal.Parse(((Label)e.Row.Cells[13].FindControl("lblCNGTk")).Text);
                    

                    totalferry += decimal.Parse(((Label)e.Row.Cells[14].FindControl("lblFerryExp")).Text);
                    totalbridge += decimal.Parse(((Label)e.Row.Cells[15].FindControl("lblBridgeExp")).Text);

                    totalmaintenance += decimal.Parse(((Label)e.Row.Cells[17].FindControl("lblMaintenance")).Text);

                    totalpolice += decimal.Parse(((Label)e.Row.Cells[18].FindControl("lblpolice")).Text);
                    totallabour += decimal.Parse(((Label)e.Row.Cells[19].FindControl("lblLabour")).Text);

                    totalOther += decimal.Parse(((Label)e.Row.Cells[21].FindControl("lblOther")).Text);
            ...
            try
            {
                ...
                if (hdnconfirm.Value == "1")
                {
                    ...
                    if (intReportType == 4)
                    {
                        ...
                        if (dgvVendorsTReport.Rows.Count > 0)
                        {
                            dt = new DataTable();
                            dt = obj.GetDateWiseCompleteReport(intWork, intShipPoint, dteFromDate, dteToDate, intFuelStationid, intBillStatus, intEnroll, intReportCategory);
                            ScriptManager.RegisterStartupScript(Page, typeof(Page), "StartupScript", "hideGridAll();", true);
        ...
        protected void ddlReportType_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            btnExport.Visible = false;
            if (ddlReportType.SelectedValue.ToString() == "4")
            {
            ...
            else if (ddlReportType.SelectedValue.ToString() == "4")
            {
                btnExport.Visible = false;
                btnExportVT.Visible = true;
            }
            else
            {
                btnExport.Visible = false;
        ...
        {           
            ...
            ExportToExcel(ref html, "InternalTransport");
        }
        ...
        protected void btnExportVT_Click(object sender, EventArgs e) 
        {            
            string html = HdnValue.Value;
            ExportToExcelVT(ref html, "VendorTransport");
        }
        public void ExportToExcelVT(ref string html, string fileName)
        {
            html = html.Replace("&gt;", ">");
            html = html.Replace("&lt;", "<");
            HttpContext.Current.Response.ClearContent();
