...
{
    public partial class BrandItemReqAprvByEventDept : System.Web.UI.Page
    {
        ...
        private void Loadgrid()
        {
            ...
            {
                //5
                dtbl = bll.CreateStoreRequisitionForBrandItem(11, int.Parse(HttpContext.Current.Session[SessionParams.USER_ID].ToString()), "", 0, DateTime.Parse(txtFDate.Text), DateTime.Parse(txtTDate.Text));
        ...
        protected void btnDetails_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                dtbl = bll.CreateStoreRequisitionForBrandItem(12, int.Parse(HttpContext.Current.Session[SessionParams.USER_ID].ToString()), "", int.Parse(senderdata), DateTime.Parse(txtFDate.Text), DateTime.Parse(txtTDate.Text));
                if (dtbl.Rows.Count > 0)
                {
        ...
        protected void btnApp_Click(object sender, EventArgs e)
        {
            try
            {
                if (dgvissue.Rows.Count > 0 && hdnconfirm.Value == "1")
                {
                    ...
                    string trid = reqid;
                    dtbl = bll.CreateStoreRequisitionForBrandItem(13, int.Parse(HttpContext.Current.Session[SessionParams.USER_ID].ToString()), xmlString, int.Parse(reqid), DateTime.Now, DateTime.Now);
                    //7
                    string sts = "1";
                    sts = dtbl.Rows[0]["Messages"].ToString();
                    File.Delete(xmlpath);
                    #region -------------Loadgrid -------------------
                    dtbl = bll.CreateStoreRequisitionForBrandItem(12, int.Parse(HttpContext.Current.Session[SessionParams.USER_ID].ToString()), "", int.Parse(reqid), DateTime.Parse(txtFDate.Text), DateTime.Parse(txtTDate.Text));
                    if (dtbl.Rows.Count > 0)
                    {
                        dgvissue.DataSource = dtbl; dgvissue.DataBind();
                        lblRN.Text = dtbl.Rows[0]["Code"].ToString();
