...
{
    public partial class RAWMaterialTransfer :BasePage
    {
        XmlManagerSO xm = new XmlManagerSO(); ItemPrice objbll = new ItemPrice(); bool ysnvisible;
        TourPlanning brandch = new TourPlanning();
        //SAD_DAL.Sales
        //TourPlanningTDS.QryBrandItemChallanCustomerDataTable table;
        ...
        protected override void OnPreInit(EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                if (Request.QueryString["id"] != null)
                {
                    ...
                    //table = tp.GetBrandItemEntry(Request.QueryString["id"]);
                    ...
                    if (table.Rows.Count > 0)
                    {
                        ...
                        //SAD_DAL.Sales
                        //TourPlanningTDS.TblBrandItemChallanDetDataTable tbl = tp.GetBrandChallanDetails(table[0].intId.ToString());
                        SalesEntryTDS.QrySalesEntryDetailsDataTable tbl = se.GetSalesEntryDetails(table[0].intId.ToString());
                        for (int i = 0; i < tbl.Rows.Count; i++)
                        {
                            string[][] items = xm.CreateItemsraw(tbl[i].intProductId.ToString()
                                , tbl[i].strProductName
                                , tbl[i].numQuantity.ToString()
                                , tbl[i].intUom.ToString()
                                , tbl[i].strUOM.ToString()
                                , tbl[i].strNarration
                               , tbl[i].monPrice.ToString()
                                );

                            XmlDocument xmlDoc = xm.LoadXmlFileraw(GetXmlFilePath());
                    ...
                    if (table.Rows.Count > 0)
                    {
                        ...
                        if (table[0].ysnLogistic)
                        {
                            ...
                            else
                            {
                                ...
                                //BLL.Accounts.ChartOfAccount
                                ChartOfAcc coa = new ChartOfAcc();
                                //DAL.Accounts.ChartOfAccount
        ...
        public static string[] GetProductList(string prefixText, int count)
        {
            ...
            //SAD_BLL.Item
            return TourPlanning.GetRAWMaterialProductDataForAutoFill(HttpContext.Current.Session[SessionParams.CURRENT_UNIT].ToString(), prefixText);
        }
        ...
        protected void btnAdd_Click(object sender, EventArgs e)
        {
            ...
            if ( txtQun.Text.Trim() != "") 
            {
                ...
                string[][] items = xm.CreateItemsraw(hdnProduct.Value, hdnProductText.Value
                    , txtQun.Text, ddlUOM.SelectedValue, ddlUOM.SelectedItem.Text,
                   narr, lblPrice.Text.ToString());
        ...
        protected void btnSubmit_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                for (int i = 0; i < GridView1.Rows.Count; i++)
                {
                    if (GridView1.Rows[i].RowType == DataControlRowType.DataRow)
                    {
                        narrTop += "[" + (GridView1.Rows[i].Cells[1].Text + "]");
                    }
                }

                TourPlanning tp = new TourPlanning();
                tp.AddbranditemchallanInformation(xml, Session["sesUserID"].ToString(), ddlUnit.SelectedValue
                    , CommonClass.GetDateAtSQLDateFormat(txtDate.Text + " 09:00 AM"), txtChallan.Text.Trim(), hdnCustomer.Value
                    , ddlCusType.SelectedValue
                    , narrTop, txtAddress.Text.Trim()
                    ...
                    , "oth", txtDriver.Text.Trim(), txtDriverContact.Text.Trim(), ddlSo.SelectedValue, ddlShip.SelectedValue, ref code, ref id);
                ...
                string genchallan = code.ToString();
                lblCreatedChallan.Text = genchallan;
                ScriptManager.RegisterStartupScript(Page, typeof(Page), "StartupScript", "alert(' Succesfully created . Challan No :   " + genchallan + "');", true);
        ...
        private string GetXmlFilePath()
        {
            ...
            return Server.MapPath("~/Inventory/Data/BrandItemsISSUEREQ_" + HttpContext.Current.Session[SessionParams.USER_ID].ToString() + "_item.xml");
            //return Server.MapPath("") + "/Data/" + Session[SessionParams.USER_ID] + "_" + unit + "_item.xml";
        }
        private void BindGrid(string xmlFilePath)
        {
            if (!File.Exists(xmlFilePath)) xm.LoadXmlFileraw(xmlFilePath);
