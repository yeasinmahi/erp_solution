...
namespace UI.Inventory
{
    public partial class SupplierEnlistment : BasePage
    {
        ...
        string filePathForXMLDocUpload; string xmlStringDocUpload = ""; string xmlDocUpload; string xml;
        string strDocUploadPath; int intDocType; string strFilePath; string strDocName;
        string fileName, docType; string doctypeid; string strFileName; string strSubmitDate;
        int intID, intInsertBy; DateTime dteSubmitDate;
            ...
            if (hdnconfirm.Value == "3")
            {
                try
                {
                    ...
                    else if (strRoutingNo.Length != 9)
                    {
                        ...
                        ScriptManager.RegisterStartupScript(Page, typeof(Page), "StartupScript", "alert('Routing no Must be 9 digit ??');", true);
                        ScriptManager.RegisterStartupScript(Page, typeof(Page), "close", "OpenHdnDiv();", true);
                    }
                    ...
                    else if (strPayToName != "" && strRoutingNo.Length == 9 && strACNO.Length == 13 && strReprContactNo.Length == 11)
                    {
                        ...
                        if(docType=="1") // Cheque Statement
                        {
                            ...
                            string message = Enlist.InsertSuppEnlistment(strSuppMasterName, strOrgAddress, strOrgMail, strOrgContactNo, strOrgFAXNo, strBusinessType, strServiceType, strBIN, strTIN, strVATRegNo, strTradeLisenceNo, strReprName, strReprContactNo, strPayToName, strSupplierType, dteEnlistmentDate, intRequestBy, strShortName, strACNO, strRoutingNo, strBank, strBranch, intBankID, intDistrictID, intBranchID, xml);
                        ...
                        {
                            ScriptManager.RegisterStartupScript(Page, typeof(Page), "StartupScript", "alert('Please Upload Cheque Leave Or Bank Statement ??');", true);
                        }
